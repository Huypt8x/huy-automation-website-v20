...
export default function SystemPage({params}:{params:{slug:string}}){
  ...
  return <main className="bg-bg text-dark"><Header/>
    ...
    <section className="pb-24"><div className="section-container grid gap-8 lg:grid-cols-2"><Card label="Team" title="Ai đang vận hành hệ thống này?"><div className="mt-6 grid gap-3">{emps.map(e=>{const p=employeePromise(e); return <Link href={`/employees/${e.slug}`} className="rounded-2xl bg-bg p-5 hover:bg-blue-50" key={e.slug}><p className="text-xl font-black">{e.name} — {e.role}</p><p className="mt-2 text-sm leading-6 text-muted">{p.headline}</p></Link>})}</div></Card><Card label="Powered By" title="Workflow phía sau"><div className="mt-6 grid gap-3">{wfs.map(w=>{const copy=getWorkflowSalesCopy(w); return <Link href={`/automation/${w.slug}`} className="rounded-2xl bg-bg p-5 hover:bg-blue-50" key={w.id}><p className="font-black">{w.id} — {copy.plainName}</p><p className="mt-2 text-sm leading-6 text-muted">{copy.headline}</p></Link>})}</div></Card></div></section>
