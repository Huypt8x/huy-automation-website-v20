...
function EmployeePage({ emp }: { emp: typeof employees[number] }) {
  ...
  return <main className="bg-bg text-dark"><Header />
    ...
    <section className="pb-24"><div className="section-container grid gap-8 lg:grid-cols-[1.1fr_.9fr]"><Card label="Tôi làm gì?" title={`Nhiệm vụ của ${emp.name}`}><div className="mt-6 space-y-4">{emp.missions.map((m)=><p key={m} className="rounded-2xl bg-bg p-4 font-semibold leading-7">✓ {m}</p>)}</div></Card><Card label="Tôi tạo ra gì?" title="Output khách nhìn thấy"><div className="mt-6 space-y-4">{promise.output.map((m)=><p key={m} className="rounded-2xl bg-blue-50 p-4 font-black text-primary">✓ {m}</p>)}</div></Card></div></section>
    <section id="video" className="bg-white py-20"><div className="section-container grid gap-8 lg:grid-cols-[1fr_.85fr]"><div className="rounded-[32px] border border-border bg-bg p-8"><span className="badge">Video giới thiệu</span><h2 className="mt-5 text-3xl font-black">{emp.name} tự giới thiệu</h2>{emp.media?.find((item)=>item.type==='video') ? <div className="mt-6 overflow-hidden rounded-[28px] border border-border bg-black"><video className="aspect-video w-full bg-black object-contain" src={emp.media.find((item)=>item.type==='video')?.url} controls preload="metadata" /></div> : <div className="mt-6 flex aspect-video items-center justify-center rounded-[28px] bg-dark p-8 text-center text-white"><div><p className="text-3xl font-black">Video Avatar</p><p className="mt-3 text-sm leading-6 text-slate-300">Video giới thiệu sẽ trình bày: {emp.name} AI Employee này có thể giúp doanh nghiệp làm gì và cách đăng ký demo.</p></div></div>}{emp.media?.find((item)=>item.type==='video')?.caption && <p className="mt-4 text-sm leading-6 text-muted">{emp.media.find((item)=>item.type==='video')?.caption}</p>}</div><Card label="Hệ thống hỗ trợ" title="Automation hỗ trợ tôi"><div className="mt-6 grid gap-2">{relatedWf.map((w)=><Link key={w.id} href={`/automation/${w.slug}`} className="rounded-2xl bg-bg p-4 font-bold hover:bg-blue-50">{w.id} — {w.name}</Link>)}</div><h3 className="mt-8 text-xl font-black">Gói giải pháp</h3><div className="mt-4 grid gap-2">{relatedSystems.map((s)=><Link key={s.id} href={`/systems/${s.slug}`} className="rounded-2xl bg-bg p-4 font-bold hover:bg-blue-50">{s.id} — {s.name}</Link>)}</div></Card></div></section>
    <section className="bg-dark py-20 text-white"><div className="section-container text-center"><h2 className="text-4xl font-black">Muốn tuyển {emp.name} vào doanh nghiệp của bạn?</h2><p className="mx-auto mt-4 max-w-2xl text-slate-300">Gửi yêu cầu để nhận demo, workflow phía sau và lộ trình triển khai AI Employee này.</p><Link href={`/consultation?source=${emp.slug}`} className="mt-8 inline-flex rounded-full bg-white px-7 py-4 font-black text-dark">Tôi muốn sở hữu {emp.name}</Link></div></section><Footer /></main>;
}
